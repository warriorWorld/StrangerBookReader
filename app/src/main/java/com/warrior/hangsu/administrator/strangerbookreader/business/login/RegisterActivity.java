package com.warrior.hangsu.administrator.strangerbookreader.business.login;
...
public class RegisterActivity extends BaseActivity implements View.OnClickListener {
    ...
    private boolean checkData() {
        if (TextUtils.isEmpty(userIdEt.getText().toString())) {
            ToastUtils.showSingleToast("请输入用户名!");
            return false;
        }
        if (TextUtils.isEmpty(emailEt.getText().toString())) {
            ToastUtils.showSingleToast("请输入邮箱地址!");
            return false;
        }
        if (!StringUtil.isEmail(emailEt.getText().toString())) {
            ToastUtils.showSingleToast("请输入正确的邮箱地址!");
            return false;
        }
